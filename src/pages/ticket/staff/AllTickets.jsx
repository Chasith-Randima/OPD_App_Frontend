...
import { allTicketsRelated, allTicketsStaff } from "actions/ticket";
import SearchDoctorTickets from "components/search/SearchDoctorTickets";
import { BsFillFilterSquareFill } from "react-icons/bs";
import Message from "components/Message";
import SearchStaffTickets from "components/search/SearchStaffTickets";
...
const AllTickets = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (localStorage.getItem("user")) {
      hospitalId = JSON.parse(localStorage.getItem("user")).hospitals[0]._id;
      hospitalIdTemp = hospitalId;
    }
    setAlert({ ...alert, loading: true });
    // let field_name = "doctors";
    // let id = query.Doctor;
    let hospitals = hospitalIdTemp;
    let params = {
      limit,
      page,
      active: true,
    ...
    <>
      <Layout>
        ...
        <div className="mt-2 mb-2 p-2 mr-10 border-2 border-gray-200 rounded-xl grid grid-cols-4">
          {/* <SearchDoctorTickets doctorId={query.Doctor} /> */}
          <SearchStaffTickets hospitalId={hospitalId} />
          <div
        ...
        {show ? (
          ...
          <div className="mt-10 mr-10 border-2 border-gray-200 rounded-xl">
            <div className="grid grid-cols-7 md:grid-cols-9 bg-gray-600 p-4 rounded-xl text-white text-xl font-sb ">
              <h2>Id</h2>
              <h2 className="col-span-1 md:col-span-2">Diagnosis Title</h2>
              <h2>Date</h2>
              <h2>Time</h2>
            ...
            <div className="">
              {allData &&
                allData.doc.map((ticket) => {
                  return (
                    <Ticket ticket={ticket} key={ticket._id} type={true} />
                  );
                })}
            </div>
          </div>
...
//     // id,
//   };
//   return await allTicketsStaff(data)
//     .then((data) => {
//       console.log(data);
//       return { data, query };
//     })
